...
function getWidth(start: number, end: number) {
  const element = document.getElementById("elementsSlider") as HTMLElement;
  if (!element) {
...
function getLeftMargin(start: number) {
  ...
  const offset = element.offsetWidth / 23 * start
  return offset;
}
...
const MapAnimation = ({ username, userToken }: { username: string, userToken: string }) => {
  ...
  useEffect(() => {
    async function getGraph() {
      ...
      const body = data.graphData as boolean[];
      let prev = 0;
      let work = body[0];
      console.log(body);
      const workArray = []
  ...
  useEffect(() => {
    function resizeElements() {
      ...
      for (let i = 0; i < elems.length; i++) {
        const tempElem = elems[i] as HTMLElement;
        for (const className of tempElem.classList) {
          if (className.includes(",")) {
            const s = className.split(",")
          ...
          <div className="flex justify-between items-center">
            ...
            {Array(24).fill(1).map((_, index) => (
              <div className={`flex flex-col items-center w-[18px] ${index % 6 != 0 ? 'mt-[4px] mb-auto' : ''}`}>
                <div key={index} className={`column bg-[#D9D9D9] w-[6px] transition-all duration-500 ease-in-out ${index % 6 == 0 ? "h-[20px]" : "h-[12px]"} ${curHour == index ? 'opacity-100 bg-[#FFFFFF] shadow-2xl shadow-white' : "opacity-20"}`}></div>
